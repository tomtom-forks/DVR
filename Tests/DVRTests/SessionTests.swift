...
class SessionTests: XCTestCase {
    ...
    override func setUp() {
        ...
        session = {
            ...
            return Session(
                cassetteName: "example", testBundle: testBundle, backingSession: backingSession)
        }()
    }
    ...
    func testDataTask() {
        ...
        if let dataTask = dataTask as? SessionDataTask,
            let headers = dataTask.request.allHTTPHeaderFields
        {
    ...
    func testTextPlayback() {
        let session = Session(cassetteName: "text", testBundle: testBundle)
        session.recordingEnabled = false
    ...
    func testTextPlaybackWithAllParamsWithoutIgnoredParameter() {
        let session = Session(cassetteName: "response-headers", testBundle: testBundle)

        let request = URLRequest(
            url: URL(string: "https://httpbin.org/response-headers?apiKey=val&format=json")!)
        let expectation = self.expectation(description: "Network")
    ...
    func testTextPlaybackWithAllParamsWithIgnoredParameter() {
        let session = Session(
            cassetteName: "response-headers-without-apikey", testBundle: testBundle,
    ...
    func testDownload() {
        ...
        let session = Session(cassetteName: "json-example", testBundle: testBundle)
        session.recordingEnabled = false
    ...
    func testMultiple() {
        let expectation = self.expectation(description: "Network")
        let session = Session(cassetteName: "multiple", testBundle: testBundle)
        ...
        session.dataTask(
            with: URLRequest(url: URL(string: "http://apple.com")!),
            completionHandler: { _, response, _ in
                XCTAssertEqual(200, (response as? Foundation.HTTPURLResponse)?.statusCode)

                DispatchQueue.main.async {
                    session.dataTask(
                        with: URLRequest(url: URL(string: "http://google.com")!),
                        completionHandler: { _, response, _ in
                            XCTAssertEqual(
                                200, (response as? Foundation.HTTPURLResponse)?.statusCode)
                            google.fulfill()
                        }
                    ).resume()
    ...
    func testTaskDelegate() {
        class Delegate: NSObject, URLSessionTaskDelegate {
            ...
            @objc fileprivate func urlSession(
    ...
    func testRecordingStatusCodeForFailedRequest() {
        ...
        let request = URLRequest(
            url: URL(string: "http://cdn.contentful.com/spaces/cfexampleapi/entries")!)

        let config = URLSessionConfiguration.default
        let backingSession = URLSession(configuration: config)
        let session = Session(
            cassetteName: "failed-request-example", testBundle: testBundle,
    ...
    func testSameRequestWithDifferentHeaders() {
        ...
        let backingSession = URLSession(configuration: configuration)
        let session = Session(
            cassetteName: "different-headers", testBundle: testBundle,
            backingSession: backingSession, headersToCheck: ["Foo"])
        session.recordingEnabled = false
    ...
    func testTextPlaybackWithParams() throws {
        let session = Session(
            cassetteName: "text-with-param", testBundle: testBundle, parametersToIgnore: ["key"])
        session.recordingEnabled = false

        var request = URLRequest(
            url: URL(string: "http://example.com?status=Available&key=Wmw0860")!)
        request.httpMethod = "POST"
        request.httpBody = "Some text.".data(using: String.Encoding.utf8)
        request.setValue("text/plain", forHTTPHeaderField: "Content-Type")
    ...
    func testErrorOnMissingCassette() {
        ...
        let session = Session(
            cassetteName: uniqueUnavailableCasseteName, testBundle: testBundle,
            parametersToIgnore: ["key"])
